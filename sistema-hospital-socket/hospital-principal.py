...
def handle_client(connection):
    ...
    if choice == 1:
        ...
        cadastro_sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        cadastro_sock.connect((CADASTRO_HOST, CADASTRO_PORT))
        cadastro_data = f'{nome},{cpf},{data_nasc}'
        cadastro_sock.sendall(cadastro_data.encode())

        # Recebe a resposta do servidor de cadastro
        cadastro_response = cadastro_sock.recv(1024).decode().strip()
